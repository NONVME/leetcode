"""
https://leetcode.com/problems/maximum-product-difference-between-two-pairs/?envType=daily-question&envId=2023-12-18

The product difference between two pairs (a, b) and (c, d) is defined as (a * b) - (c * d).
For example, the product difference between (5, 6) and (2, 7) is (5 * 6) - (2 * 7) = 16.
Given an integer array nums, choose four distinct indices w, x, y, and z such that the product difference between pairs (nums[w], nums[x]) and (nums[y], nums[z]) is maximized.
Return the maximum such product difference.


Example 1:
Input: nums = [5,6,2,7,4]
Output: 34
Explanation: We can choose indices 1 and 3 for the first pair (6, 7) and indices 2 and 4 for the second pair (2, 4).
The product difference is (6 * 7) - (2 * 4) = 34.

Example 2:
Input: nums = [4,2,5,9,7,4,8]
Output: 64
Explanation: We can choose indices 3 and 6 for the first pair (9, 8) and indices 1 and 5 for the second pair (2, 4).
The product difference is (9 * 8) - (2 * 4) = 64.

Constraints:

4 <= nums.length <= 104
1 <= nums[i] <= 104
"""
from typing import List


class Solution:
    """
    T-O(nlogn)
    S-O(n)
    """
    def maxProductDifference(self, nums: List[int]) -> int:
        sorted_array = sorted(nums)
        return (sorted_array[-1] * sorted_array[-2]) - (sorted_array[0] * sorted_array[1])


class Solution1:
    """
    T-O(n)
    S-O(1)
    """
    def maxProductDifference(self, nums: List[int]) -> int:
        min_1, min_2 = (max(nums),) * 2
        max_1, max_2 = (min(nums),) * 2
        for i in nums:
            if i >= max_1 and i <= max_2:
                max_1 = i
            if i > max_1 and i > max_2:
                max_1 = max_2
                max_2 = i
            if i >= min_1 and i <= min_2:
                min_2 = i
            if i < min_1 and i < min_2:
                min_2 = min_1
                min_1 = i
        return (max_1 * max_2) - (min_1 * min_2)


class Solution2:
    """
    TODO: solve the problem using a heap
    """
    def maxProductDifference(self, nums: List[int]) -> int:
        pass


for sol in Solution1, Solution:
    assert sol().maxProductDifference([5,6,2,7,4]) == 34
    assert sol().maxProductDifference([4,2,5,9,7,4,8]) == 64
    assert sol().maxProductDifference([1,6,7,5,2,4,10,6,4]) == 68
    assert sol().maxProductDifference([2,9,5,9,1]) == 79
    assert sol().maxProductDifference([4,10,2,2,8,2,9,3,3]) == 86
